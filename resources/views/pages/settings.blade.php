<x-app-layout>
<x-slot name="title">Settings — FruitIQ</x-slot>
...
<div class="grid grid-cols-1 lg:grid-cols-4 gap-6 fade-up delay-1"
    ...
    <div class="card p-3 h-fit lg:sticky lg:top-6">
        @foreach([['profile','👤','Profile Settings'],['notifications','🔔','Notifications'],['appearance','🎨','Appearance'],['language','🌐','Language & Region'],['backup','💾','Data Backup'],['system','⚙️','System Info']] as [$t,$i,$l])
        <button @click="tab='{{ $t }}'"
    ...
    <div class="lg:col-span-3 space-y-5">
            ...
            <div class="p-6">
                <div class="flex items-center gap-6 mb-8">
                    <div class="relative">
                        ...
                        <button class="absolute -bottom-2 -right-2 w-7 h-7 g-blue rounded-full flex items-center justify-center text-white shadow-md text-xs">✏️</button>
                    </div>
                    <div>
                        <h4 class="font-black text-gray-900 text-[18px]">Juan Dela Cruz</h4>
                        <p class="text-gray-500 text-[13px] mt-0.5">Owner · FruitIQ Davao City</p>
                        <span class="badge badge-violet text-[11px] mt-2">👑 Administrator</span>
                    </div>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @foreach([['First Name','text','Juan',''],['Last Name','text','Dela Cruz',''],['Email Address','email','juan@fruitiq.ph','md:col-span-2'],['Phone Number','tel','+63 912 345 6789',''],['Business Name','text','FruitIQ Davao',''],['Location','text','Davao City, Philippines',''],['Role','text','Owner · Administrator',''],['Account Created','text','January 15, 2026','md:col-span-2']] as [$l,$t,$v,$c])
        ...
        <div x-show="tab==='notifications'" class="card overflow-hidden">
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        @foreach([['notifLow','⚠️ Low Stock Alerts','Alert when fruit stock falls below minimum level','notifLow'],['notifSpoil','🔴 Spoilage Risk Alerts','Alert when spoilage risk exceeds threshold','notifSpoil'],['notifForecast','🔮 AI Forecast Updates','Alert when new AI forecast is ready','notifForecast']] as $n)
                        <div class="flex items-center justify-between p-4 bg-[#F9F8FF] rounded-2xl border border-violet-100">
                            <div><p class="text-[13.5px] font-semibold text-gray-800">{{ $n[1] }}</p><p class="text-[12px] text-gray-400 mt-0.5">{{ $n[2] }}</p></div>
                            <button @click="{{ $n[3] }} = !{{ $n[3] }}" :class="{{ $n[3] }} ? 'g-violet' : 'bg-gray-300'" class="relative w-12 h-6 rounded-full transition-all duration-200 flex-shrink-0 shadow-inner">
                                <span :class="{{ $n[3] }} ? 'translate-x-6' : 'translate-x-1'" class="absolute top-1 w-4 h-4 bg-white rounded-full shadow-md transition-transform duration-200"></span>
        ...
        <div x-show="tab==='appearance'" class="card overflow-hidden">
            <div class="px-6 py-5 border-b border-gray-100"><h3 class="font-bold text-gray-900 text-[16px]">Appearance</h3><p class="text-[12px] text-gray-400 mt-0.5">Customize the look and feel of FruitIQ</p></div>
            <div class="p-6 space-y-6">
                <div class="flex items-center justify-between p-4 bg-[#F9F8FF] rounded-2xl border border-violet-100">
                    <div class="flex items-center gap-3"><span class="text-2xl">🌙</span><div><p class="font-semibold text-gray-800 text-[13.5px]">Dark Mode</p><p class="text-[12px] text-gray-400">Switch to dark theme</p></div></div>
        ...
        <div x-show="tab==='language'" class="card overflow-hidden">
            ...
            <div class="p-6 space-y-4">
                @foreach([['System Language','🇵🇭'],['Date Format','MM/DD/YYYY'],['Currency','₱ Philippine Peso (PHP)'],['Time Zone','Asia/Manila (UTC+8)']] as $i => [$l,$d])
                <div>
            ...
            <div class="p-6 space-y-4">
                <div class="bg-green-50 border border-green-200 rounded-2xl p-5">
                    <div class="flex items-start gap-4">
                        <div class="stat-ring g-green w-12 h-12 rounded-xl shadow-md flex-shrink-0"><span class="text-xl">💾</span></div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <button class="flex items-center justify-center gap-2.5 p-4 bg-[#F5F3FF] rounded-2xl border border-violet-100 hover:bg-violet-100 transition-all text-[13px] font-semibold text-violet-700"><span>☁️</span>Backup Now</button>
                    <button class="flex items-center justify-center gap-2.5 p-4 bg-[#F5F3FF] rounded-2xl border border-violet-100 hover:bg-violet-100 transition-all text-[13px] font-semibold text-violet-700"><span>🔄</span>Restore Backup</button>
                </div>
                <div>
        ...
        <div x-show="tab==='system'" class="card overflow-hidden">
            <div class="px-6 py-5 border-b border-gray-100"><h3 class="font-bold text-gray-900 text-[16px]">System Information</h3><p class="text-[12px] text-gray-400 mt-0.5">Technical details about your FruitIQ installation</p></div>
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    @foreach([['System Name','FruitIQ — Fruit Inventory System'],['Version','v2.0.0-beta (Build 2026.06.23)'],['Framework','Laravel 12.x'],['PHP Version','8.2.x'],['Database','SQLite 3.x'],['AI Model Version','FruitIQ-ML v2.4.1'],['Frontend','Tailwind CSS v4 + Alpine.js v3'],['License','Academic / Capstone Project']] as [$l,$v])
