<x-app-layout>
<x-slot name="title">Reports — FruitIQ</x-slot>
...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-7 fade-up">
    ...
    <div class="flex items-center gap-2.5">
        <button class="btn btn-outline btn-md text-[12.5px]">📄 PDF</button>
        <button class="btn btn-outline btn-md text-[12.5px]">🖨 Print</button>
        <button class="btn btn-outline btn-md text-[12.5px]">📊 Excel</button>
...
<div class="card overflow-hidden fade-up delay-1" x-data="{ tab: 'sales' }">
    ...
    <div class="flex gap-1 p-2 bg-gray-50 border-b border-gray-100">
        @foreach([['sales','💰 Sales'],['forecast','🔮 Forecast'],['inventory','📦 Inventory'],['spoilage','⚠️ Spoilage'],['profit','💎 Profit']] as [$t,$l])
        <button @click="tab='{{ $t }}'"
                :class="tab==='{{ $t }}' ? 'g-violet text-white shadow-md' : 'text-gray-500 hover:bg-white hover:shadow-sm'"
                class="flex-1 py-2.5 px-3 rounded-xl text-[12.5px] font-semibold transition-all">{{ $l }}</button>
        @endforeach
    </div>
    ...
    <div x-show="tab==='sales'" class="p-6">
        ...
        <table class="tbl w-full">
            ...
            <tr><td class="font-medium text-gray-800">{{ $r[0] }} {{ $r[1] }}</td><td class="text-gray-500 text-[13px]">{{ $r[2] }}</td><td class="font-bold text-gray-900">{{ $r[3] }}</td><td class="text-gray-500">{{ $r[4] }}</td><td class="font-black text-[16px] {{ $r[5]==='↑'?'text-green-600':($r[5]==='↓'?'text-red-500':'text-blue-600') }}">{{ $r[5] }}</td></tr>
            @endforeach
            </tbody>
        </table>
    </div>
    ...
    <div x-show="tab==='forecast'" class="p-6">
        ...
        <table class="tbl w-full">
            ...
            <tr><td class="font-medium text-gray-800">{{ $r[0] }} {{ $r[1] }}</td><td class="text-gray-500">{{ $r[2] }}</td><td class="text-gray-500">{{ $r[3] }}</td><td class="font-semibold {{ str_starts_with($r[4],'-')?'text-green-600':'text-amber-600' }}">{{ $r[4] }}</td><td><span class="badge badge-green text-[11px]">{{ $r[5] }}</span></td></tr>
            @endforeach
            </tbody>
        </table>
    </div>
    ...
    <div x-show="tab==='inventory'" class="p-6">
        ...
        <table class="tbl w-full">
            ...
            <tr><td class="font-medium text-gray-800">{{ $r[0] }} {{ $r[1] }}</td><td class="text-gray-500">{{ $r[2] }}</td><td class="text-gray-500">{{ $r[3] }}</td><td class="font-semibold text-green-600">{{ $r[4] }}</td><td class="text-gray-500">{{ $r[5] }}</td><td class="font-black text-violet-700">{{ $r[6] }}</td></tr>
            @endforeach
            </tbody>
        </table>
    </div>
    ...
    <div x-show="tab==='spoilage'" class="p-6">
        ...
        <table class="tbl w-full">
            ...
            <tr><td class="font-medium text-gray-800">{{ $r[0] }} {{ $r[1] }}</td><td class="text-red-600 font-semibold">{{ $r[2] }}</td><td class="font-bold text-gray-900">{{ $r[3] }}</td><td>
                <div class="flex items-center gap-2"><div class="progress-bar w-12"><div class="{{ (int)$r[4]>30?'bg-red-500':((int)$r[4]>15?'bg-amber-500':'bg-green-500') }} h-full rounded-full" style="width:{{ min((int)$r[4],100) }}%"></div></div><span class="text-[12.5px] font-bold text-gray-800">{{ $r[4] }}</span></div>
            </td><td class="text-gray-500 text-[12.5px]">{{ $r[5] }}</td><td><span class="badge {{ $r[6] }} text-[11px]">{{ (int)$r[4]>30?'Critical':((int)$r[4]>10?'Moderate':'Low Risk') }}</span></td></tr>
            @endforeach
            </tbody>
        </table>
    </div>
