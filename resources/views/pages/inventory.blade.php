<x-app-layout>
<x-slot name="title">Inventory — FruitIQ</x-slot>
...
@foreach([['Total Stock','1,240 kg','📦','g-violet','All batches'],['Available','1,102 kg','✅','g-green','Ready to sell'],['Critical Items','3 items','🔴','g-rose','Needs restock'],['Out of Stock','1 item','⛔','g-amber','Depleted']] as [$l,$v,$i,$g,$s])
...
@endphp
@foreach($items as $it)
<div class="card overflow-hidden card-lift fade-up delay-{{ min($loop->index+1,8) }}" x-on:click="viewModal=true; selected='{{ $it[2] }}'">
    <div class="{{ $it[9] }} p-5 relative">
        <div class="absolute top-3 right-3"><span class="badge {{ $it[8] }} text-[10px]">{{ $it[7] }}</span></div>
        <div class="text-5xl mb-3 drop-shadow-lg">{{ $it[0] }}</div>
        <p class="font-black text-white text-[17px] leading-tight">{{ $it[1] }}</p>
        <p class="text-white/70 text-[11px] font-medium mt-0.5 font-mono">{{ $it[2] }}</p>
    </div>
    <div class="p-4">
        <div class="flex items-center justify-between mb-3">
            <div>
                <p class="text-[22px] font-black text-gray-900">{{ $it[3] }}</p>
                <p class="text-[11.5px] text-gray-400">Stock quantity</p>
            </div>
            <div class="text-right">
                <p class="text-[14px] font-bold text-violet-700">{{ $it[6] }}</p>
                <p class="text-[11px] text-gray-400">Unit price</p>
            </div>
        </div>
        <div class="progress-bar mb-3">
            <div class="progress-fill {{ $it[10] < 20 ? 'bg-red-500' : ($it[10] < 50 ? 'bg-amber-400' : 'bg-violet-600') }}" style="width:{{ $it[10] }}%"></div>
        </div>
        ...
        <div class="flex items-center justify-between">
            <span class="text-[12px] font-semibold text-gray-600">Freshness: <span class="{{ $it[10] > 70 ? 'text-green-600' : ($it[10] > 40 ? 'text-amber-600' : 'text-red-600') }}">{{ $it[11] }}</span></span>
            <div class="flex gap-1.5">
                <button @click.stop="editModal=true; selected='{{ $it[1] }}'" class="p-1.5 text-gray-400 hover:text-amber-600 hover:bg-amber-50 rounded-lg transition-colors"><svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></button>
                <button @click.stop="deleteModal=true; selected='{{ $it[1] }}'" class="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"><svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
            </div>
        </div>
    </div>
</div>
